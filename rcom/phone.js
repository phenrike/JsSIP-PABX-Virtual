/*
Configure_Account_wssServer="espelho.handphone.com.br";
Configure_Account_WebSocketPort="8089";
Configure_Account_ServerPath="/ws";
Configure_Account_profileUser="9301";
Configure_Account_profileName="9301";
Configure_Account_SipUsername="9301";
Configure_Account_SipPassword="123@handphone";
*/
const queryString = window.location.search;
console.log(queryString);
const urlParams = new URLSearchParams(queryString);
const nocache = urlParams.get('nocache');
console.log("Valor do Cache:");
console.log(nocache);
...
let welcomeScreen = "<div class=\"UiWindowField\"><pre style=\"font-size: 12px\" display:none;>";
welcomeScreen += "</div>";
...
let VoiceMailSubscribe = (getDbItem("VoiceMailSubscribe", "1") == "0");             // Enable Subscribe to voicemail
let VoicemailDid = getDbItem("VoicemailDid", "");                                  // Number to dail for VoicemialMain()
let userAgentStr = getDbItem("UserAgentStr", " Handphone " + appversion + " (SIPJS - " + sipjsversion + ")");   // Set this to whatever you want.
let hostingPrefex = getDbItem("HostingPrefex", "");                                 // Use if hosting off root directiory. eg: "/phone/" or "/static/"
let RegisterExpires = parseInt(getDbItem("RegisterExpires", 120));                  // Registration expiry time (in seconds)
...
let MaxDidLength = parseInt(getDbItem("MaxDidLength", 30));          // Maximum langth of any DID number including international dialled numbers.
...
function PreloadAudioFiles() {
    ...
    audioBlobs.Ringtone = { file: "ringtone.wav", url: hostingPrefex + "../../audios/espera/AUDIOSDAC/ringtone.wav" }
...
function ReceiveCall(session) {
    ...
    // LEANDRO linha abaixo estava  var lineObj = new Line(newLineNumber, callerID, did, buddyObj);
    ...
    if (AutoAnswerEnabled || AutoAnswerPolicy == "enabled" || autoAnswerRequested) {
        if (CurrentCalls == 0) { // There are no other calls, so you can answer
            ...
            if (popup_webrtc == "1") {
                window.open("popup.php?auto=" + AutoAnswerEnabled + "", "RECEBENDO CHAMADA", "width=817,height=612,right=100%,top=100%,statusbar=no,resizable=no,toolbar=0");
            }
    ...
    //botoes do toast
    var htmltoast = "";
    htmltoast += "<center><button onclick=\"AnswerAudioCall('" + lineObj.LineNumber + "'); fechatoast();\" class=answerButton><i class=\"fa fa-phone\"></i> " + lang.answer_call + "</button> ";
    if (EnableVideoCalling && atender_com_video == "1") {
        htmltoast += " <button id=\"line-" + lineObj.LineNumber + "-answer-video\" onclick=\"AnswerVideoCall('" + lineObj.LineNumber + "');fechatoast();\" class=answerButton><i class=\"fa fa-video-camera\"></i> " + lang.answer_call_with_video + "</button><p></p> ";
    }
    htmltoast += " <button onclick=\"RejectCall('" + lineObj.LineNumber + "'); fechatoast();\" class=rejectButton><i class=\"fa fa-phone\" style=\"transform: rotate(135deg);\"></i> " + lang.reject_call + "</button></center>";
    $("#toastinfo").append(htmltoast);
    $("#idchamada").html(lineObj.DisplayNumber);
    $("#toast").show();

    // caso receba outra chamada sem responder se quer retornar
    $('#chamadacaiu').hide();
...
function onInviteProgress(lineObj, response) {
    ...
    // LEANDRO
    console.log("Classificacao de Chamada de Saida");
    classificacall();
}
...
function onSessionRecievedBye(lineObj, response) {
    ...
    teardownSession(lineObj);
    // Leandro Arbia - Retorno de Chamada
    console.log("Call ended, Perguntando se a chamada caiu!");

    mostrachamadacaiu(numero_chamada_caiu);

    //em caso de recebimento de hangup pelo outro lado 07/04/2023
    $('#mostrar_transferencia').hide();
    $('#col-sm-7').show();
    ...
    if (includeVideo) {
        ...
        if (remoteVideoStream.getVideoTracks().length >= 1) {
            ...
            remoteVideoStreamTracks.forEach(function (remoteVideoStreamTrack) {
                ...
                // chamada com video trocando CSS
                //  var elementStyle = document.getElementById("paineloperacional").style;
                //  elementStyle.position = "absolute";
                //  elementStyle.top = elementStyle.left = "0";
...
function UpdateBuddyActivity(buddy, lastAct) {
    ...
    // Leandro Arbia - Chamada Caiu ?
    numero_chamada_caiu = buddyObj.CallerIDName;
...
function endSession(lineNum) {
    ...
    // Leandro Arbia Acrescentado para finalizar chamada de saida em caso de desistencia
    cancelSession(lineNum);
...
function ShowDial() {
    ...
    //var html = "<div style=\"text-align:right\"><button class=roundButtons onclick=\"ShowContacts()\"><i class=\"fa fa-close\"></i></button></div>"
    var html = "";
    html += "<div style=\"text-align:center; margin-top:15px;\"><input placeholder=\"destino\" id=dialText class=dialTextInput oninput=\"handleDialInput(this, event)\" onkeydown=\"dialOnkeydown(event, this)\" style=\"width:200px; height:32px; font-size:14px; background-color:#CDCDCD\"><button id=dialDeleteKey class=roundButtons onclick=\"KeyPress('del')\"><</button></div><p></p>";
    /*  html += "<table cellspacing=10 cellpadding=0 style=\"margin-left:auto; margin-right: auto\">";
...
function AddLineHtml(lineObj) {
    ...
    var htmlvideo = "";
    var html = "<table id=\"line-ui-" + lineObj.LineNumber + "\" class=stream cellspacing=0 cellpadding=0 style=\"height:330px;\">";
    ...
    // BOTOES PARA FAZER HIDE DO TRAGFEGO DE REDE E HISTORICO
    ...
    html += "<button onclick=\"AnswerAudioCall('" + lineObj.LineNumber + "');fechatoast();\" class=answerButton><i class=\"fa fa-phone\"></i> " + lang.answer_call + "</button> ";
    if (EnableVideoCalling && atender_com_video == "1") {
        html += " <button id=\"line-" + lineObj.LineNumber + "-answer-video\" onclick=\"AnswerVideoCall('" + lineObj.LineNumber + "');fechatoast();\" class=answerButton><i class=\"fa fa-video-camera\"></i> " + lang.answer_call_with_video + "</button> ";
    }
    html += " <button onclick=\"RejectCall('" + lineObj.LineNumber + "');fechatoast();\" class=rejectButton><i class=\"fa fa-phone\" style=\"transform: rotate(135deg);\"></i> " + lang.reject_call + "</button> ";
    // CRM
    ...
    // Mute
    htmlvideo += "<center>";
    htmlvideo += "<div class=CallControlContainer>";
    htmlvideo += "<div class=CallControl>";
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-Mute\" onclick=\"MuteSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.mute + "\"><i class=\"fa fa-microphone-slash\"></i></button>";
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-Unmute\" onclick=\"UnmuteSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.unmute + "\" style=\"color: red; display:none\"><i class=\"fa fa-microphone\"></i></button>";
    // Hold
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-Hold\" onclick=\"holdSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\"  title=\"" + lang.hold_call + "\"><i class=\"fa fa-pause-circle\"></i></button>";
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-Unhold\" onclick=\"unholdSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.resume_call + "\" style=\"color: red; display:none\"><i class=\"fa fa-play-circle\"></i></button>";

    // Transfer (Audio Only)
    if (EnableTransfer) {
        htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-Transfer\" onclick=\"StartTransferSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.transfer_call + "\"><i class=\"fa fa-reply\" style=\"transform: rotateY(180deg)\"></i></button>";
        htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-CancelTransfer\" onclick=\"CancelTransferSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.cancel_transfer + "\" style=\"color: red; display:none\"><i class=\"fa fa-reply\" style=\"transform: rotateY(180deg)\"></i></button>";
    }
    // Expand UI (Video Only)
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-expand\" onclick=\"ExpandVideoArea('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\"><i class=\"fa fa-expand\"></i></button>";
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-restore\" onclick=\"RestoreVideoArea('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" style=\"display:none\"><i class=\"fa fa-compress\"></i></button>";
    // End Call


    html += "<div id=\"mostrar_transferencia\" style=\"display:none\"><p></p><center><button onclick=\"boxtransferencia()\" class=\"btn btn-sm btn-success\">Agentes</button><button onclick=\"boxtransferencia_grupo()\" class=\"btn btn-sm btn-success\">Grupos</button></center></div>";


    html += "<br><center><button id=\"line-" + lineObj.LineNumber + "-btn-End\" onclick=\"endSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons hangupButton\" title=\"" + lang.end_call + "\"><i class=\"fa fa-phone\" style=\"transform: rotate(135deg);\"></i></button></center>";



    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-End\" onclick=\"endSession('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons hangupButton\" title=\"" + lang.end_call + "\"><i class=\"fa fa-phone\" style=\"transform: rotate(135deg);\"></i></button>";
    ...
    if (typeof MediaRecorder != "undefined" && (CallRecordingPolicy == "allow" || CallRecordingPolicy == "enabled")) {
        ...
        htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-start-recording\" onclick=\"StartRecording('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.start_call_recording + "\"><i class=\"fa fa-dot-circle-o\"></i></button>";
        htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-stop-recording\" onclick=\"StopRecording('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.stop_call_recording + "\" style=\"color: red; display:none\"><i class=\"fa fa-circle\"></i></button>";
    }
    // Conference
    if (EnableConference) {
        htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-Conference\" onclick=\"StartConferenceCall('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.conference_call + "\"><i class=\"fa fa-users\"></i></button>";
        htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-CancelConference\" onclick=\"CancelConference('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.cancel_conference + "\" style=\"color: red; display:none\"><i class=\"fa fa-users\"></i></button>";
    }
    // DTMF
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-ShowDtmf\" onclick=\"ShowDtmfMenu('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.send_dtmf + "\"><i class=\"fa fa-keyboard-o\"></i></button>";
    // Settings
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-settings\" onclick=\"ChangeSettings('" + lineObj.LineNumber + "', this)\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.device_settings + "\"><i class=\"fa fa-volume-up\"></i></button>";
    // Present
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-present-src\" onclick=\"ShowPresentMenu(this, '" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.camera + "\"><i class=\"fa fa-video-camera\"></i></button>";
    // Call Stats
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-ShowCallStats\" onclick=\"ShowCallStats('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.call_stats + "\"><i class=\"fa fa-area-chart\"></i></button>";

    // Call Timeline
    htmlvideo += "<button id=\"line-" + lineObj.LineNumber + "-btn-ShowTimeline\" onclick=\"ShowCallTimeline('" + lineObj.LineNumber + "')\" class=\"roundButtons dialButtons inCallButtons\" title=\"" + lang.activity_timeline + "\"><i class=\"fa fa-list-ul\"></i></button>";

    htmlvideo += "</div>"; // .CallControl
    htmlvideo += "</div>"; // .CallControlContainer
    htmlvideo += "</center>";
    // FIM DOS BOTOES
    ...
    $("#paineloperacional").append(html);
    $("#paineloperacional").css("position", "relative");
    $("#paineloperacional").css("bottom", "200px");
    $("#card-aa-wpp").css("position", "relative");
    $("#card-aa-wpp").css("top", "100px");
    $("#card-aa-wpp").css("left", "380px");
    $("#aba7").append(htmlvideo);
    $(".content-footer").css("display", "none");


    //controles onde irao entrar 
}
...
function CloseLine(lineNum) {
    ...
    if (totallinhas == 0) {
        ShowDial();
        $("#aba7").html("");
        $("#paineloperacional").css("position", "");
        $("#paineloperacional").css("bottom", "");
        $("#card-aa-wpp").css("position", "");
        $("#card-aa-wpp").css("top", "");
        $("#card-aa-wpp").css("left", "");
        $(".content-footer").css("display", "flex");
    }
...
function ShowEmojiBar(buddy) {
    ...
    var emojis = ["ðŸ˜€", "ðŸ˜", "ðŸ˜‚", "ðŸ˜ƒ", "ðŸ˜„", "ðŸ˜…", "ðŸ˜†", "ðŸ˜‡", "ðŸ˜ˆ", "ðŸ˜‰", "ðŸ˜Š", "ðŸ˜‹", "ðŸ˜Œ", "ðŸ˜", "ðŸ˜Ž", "ðŸ˜", "ðŸ˜", "ðŸ˜‘", "ðŸ˜’", "ðŸ˜“", "ðŸ˜”", "ðŸ˜•", "ðŸ˜–", "ðŸ˜—", "ðŸ˜˜", "ðŸ˜™", "ðŸ˜š", "ðŸ˜›", "ðŸ˜œ", "ðŸ˜", "ðŸ˜ž", "ðŸ˜Ÿ", "ðŸ˜ ", "ðŸ˜¡", "ðŸ˜¢", "ðŸ˜£", "ðŸ˜¤", "ðŸ˜¥", "ðŸ˜¦", "ðŸ˜§", "ðŸ˜¨", "ðŸ˜©", "ðŸ˜ª", "ðŸ˜«", "ðŸ˜¬", "ðŸ˜­", "ðŸ˜®", "ðŸ˜¯", "ðŸ˜°", "ðŸ˜±", "ðŸ˜²", "ðŸ˜³", "ðŸ˜´", "ðŸ˜µ", "ðŸ˜¶", "ðŸ˜·", "ðŸ™", "ðŸ™‚", "ðŸ™ƒ", "ðŸ™„", "ðŸ¤", "ðŸ¤‘", "ðŸ¤’", "ðŸ¤“", "ðŸ¤”", "ðŸ¤•", "ðŸ¤ ", "ðŸ¤¡", "ðŸ¤¢", "ðŸ¤£", "ðŸ¤¤", "ðŸ¤¥", "ðŸ¤§", "ðŸ¤¨", "ðŸ¤©", "ðŸ¤ª", "ðŸ¤«", "ðŸ¤¬", "ðŸ¤­", "ðŸ¤®", "ðŸ¤¯", "ðŸ§"];
...
function ShowMyProfile() {
    ...
    var html = "<div style=\"text-align:right\"><button class=roundButtons onclick=\"ShowDial()\"><i class=\"fa fa-close\"></i></button></div>"
    ...
    buttons.push({
        ...
        action: function () {
            ...
            if (EnableAccountSettings) {
                localDB.setItem("wssServer", Configure_Account_wssServer);
                localDB.setItem("WebSocketPort", Configure_Account_WebSocketPort);
                localDB.setItem("ServerPath", Configure_Account_ServerPath);
                localDB.setItem("profileUser", Configure_Account_profileUser);
                localDB.setItem("profileName", Configure_Account_profileName);
                localDB.setItem("SipUsername", Configure_Account_SipUsername);
                localDB.setItem("SipPassword", Configure_Account_SipPassword);
...
// PROCESSOS COM TIMERS
function iniciando() {
    myVar = setInterval(" ShowDial(); clearInterval(myVar);", 2000);
}
iniciando();
